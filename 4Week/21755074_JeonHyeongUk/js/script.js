...
function getMaxValue(arr_things2) {
    ...
    for (i = 0; i < arr_things2.length; i++) {
        ...
        if (typeof arr_things2[i] === 'number') {
            ...
            arr_3.sort(function(a, b){return a - b}); // -> 여기서 return 값이 들어가면 안되지 않나? 생각이 들어서 스크립트를 하나 더 작성을 할까 생각을 했습니다...
...
console.log(resultMaxValue); // -> 3 ... 이 나올줄 알았지만 2가 나와버립니다..

resultMaxValue = getMaxValue([-400, "world", 60, 0, {}, 1000]);
console.log(resultMaxValue); // -> 5 ... 가 나올줄 알았지만 0이 나와버립니다..



*/ // 일단 기존의 검증 코딩의 경우 .sort() 배열 정렬을 이용한다음 정렬 맨 뒷쪽에 있는 원소를 최댓점으로 얻은 다음 해당 원소의 순서 정렬 숫자를 얻어서 반환값을 얻을려고 했습니다만, 계속해서 함수 호출 예시의 경우의 값이 모두 console에서 3과 5로 출력해야 되어야 하는데 5와 2가 출력이 됩니다. ㅠ_ㅠ 일단 혹시 몰라 미리 과제를 제출 드리며 조금 더 좋은 방법을 찾게 된다면 과제에 수정이 될 수 있도록 하겠습니다!



// ------------------------------------------------------------------
// ----- 밑에서 부터는 새롭게 작성한 검증 코딩을 나열하도록 하겠습니다 ! -----
// ------------------------------------------------------------------



// 아래에 있을 함수에서 배열에서 최댓값을 지닌 배열의 원소를 저장해 둘 임의의 변수 max_num를 선언합니다.
var max_num = 0;

// 배열에서 최댓값을 가진 원소의 정렬 순서의 숫자형을 저장해 둘 임의의 변수 arr_max_num를 선언합니다.
var arr_max_num;

function getMaxValue(arr_3)
// 전달인자 값의 배열에서 최댓값을 얻기 위한 여정을 진행할 함수 getMaxValue 선언합니다. 그 다음, 전달인자의 값을 이어 받을 임의의 매개변수 arr_3을 선언하여 함수 getMaxValue에 쓰여지도록 합니다.
{
    // 배열 arr_3의 원소들을 세기 위하여 반복문 for을 호출, 초기화 변수 i를 선언하고 값을 0으로 설정, 변수 i는 배열 arr_3의 길이보다 작도록 설정하며, 증감 연산자 ++를 설정하여 0번째 부터 있는 원소부터 (배열 arr_3의 길이 - 1)번째 원소까지 셀 수 있도록 for문의 연산 조건을 설정합니다.
    for(i = 0; i < arr_3.length; i++) 
    {

        // 아래 항목의 함수 호출 예시 리스트에서 객체형과 문자열형 값들이 존재하고 있다는 제한 조건을 착안, 이를 해결하기 위해 if 조건문을 호출하고 typeof 연산자와 일치 연산자(===)를 사용하여 배열 arr_3의 원소들의 값들이 문자형인 경우에 일치하는지에 대한 조건 여부에서 해당 조건에 true인 경우들을 불러옵니다.
        if(typeof arr_3[i] === 'number')
        {

            // 그 다음에는, 원소간에 값의 크기 차이를 비교하면서 최댓값이 어떤 것인지 정의하기 위해 if 조건문을을 다시 한번 호출하도록 하며, 최댓값이 배열 arr_3의 현재 i번째 원소보다 작다는 비교 전제를 둡니다.
            if(max_num < arr_3[i]) 

            {  // 그 다음에는, 해당 if 조건문에서 true인 경우들을 설정해주도록 하겠습니다.
                
                // 최댓값으로 저장하도록 설정한 변수 max_num에 원소 arr_3[i]를 대입시켜주도록 합니다.
                max_num = arr_3[i];
                
                // 배열상에서 최댓값의 배열 순서에 대한 숫자형을 저장해주도록 하는 변수 arr_max_num에 최댓값의 배열 순서 값을 저장하는 i가 대입되게끔 해줍니다.
                arr_max_num = i;
            }
        }
    }

    // 함수 getMaxValue의 코딩 여정이 마무리 되었습니다. 변수 arr_max_num의 값을 반환값으로 지정해 줄 수 있도록 합니다.
    return arr_max_num;
}


// 변수 resultMaxValue를 선언합니다.
var resultMaxValue;
...
resultMaxValue = getMaxValue([10, -4, 7, 100, "hello", -50]);
console.log(resultMaxValue); // -> 해당 배열에서 최댓값은 100, 해당 배열에서 '3'번째에 속해있습니다. -> 3

resultMaxValue = getMaxValue([-400, "world", 60, 0, {}, 1000]);
console.log(resultMaxValue); // -> 해당 배열에서 최댓값은 10000, 해당 배열에서 '5'번째에 속해있습니다. -> 5