...
function getEven(num1) // getEven이라는 함수를 선언하고, 1개의 매개변수(숫자)를 지정합니다.
    {
        var even = ""; // 짝수를 넣어줄 변수 even을 만들어줍니다.
        for (var i = 0; i <= num1; i++){ // for절을 만들어주고, i가 0부터 num1까지 반복되게 합니다.
            if (i % 2 === 0 && i != 0){ // 0을 빼고 짝수일 경우,
                even += i; // even에 값이 들어갑니다. 그리고 이 때
                if (i <= num1 -1){ // 만약 i가 num1 -1보다 같거나 작을 경우
                    even += ', '; // 쉼표가 들어갑니다.
                }
                else if (i == num1){ // 만약 i가 num1과 같을 경우
                    even += '.'; // 온점이 들어갑니다.
                }
            }
        }return even; // even을 반환합니다.
    }
    
    var even1 = getEven(10); // even1 변수를 선언하고 getEven(10) 의 반환값을 할당받았습니다.
...
{
    var result1=''; // 값을 받아줄 변수 result1을 선언합니다.
    if (typeof num2 !== "number"){ // 입력된 전달인자가 숫자가 아닌 경우
        console.log("숫자만 입력가능합니다.") // '숫자만 입력가능합니다.’ 라는 문구가 console 에 출력되고
        result1 = false; // false 를 반환합니다.
    }
    else if (typeof num2 === "number"){ // 입력된 전달인자가 숫자인 경우
        for (var i=1; i<=num2; i++){ 
            result1 += '*'; // 전달인자의 수만큼 '*' 가 반복되는 문자열을 반환합니다.
        }
    }
    return result1; // result1을 반환합니다.
}
...
var fruit = ['Apple', 'Orange', 'Grape', 'Melon']; // 'Apple', 'Orange', 'Grape', 'Melon' 를 요소로 가지는 fruit 라는 이름의 배열 객체를 생성합니다.
fruit.unshift('Mango'); // 'Mango' 를 fruit 배열 객체의 앞에 추가합니다.
fruit.splice(3, 0, 'Cherry'); // fruit 배열 객체의 ‘Orange' 요소 순서 뒤에 'Cherry' 요소를 추가합니다.
...
{
    var winWidth = window.innerWidth; // winWidth에 윈도우 콘텐츠의 넓이값을 넣어줍니다.
    var winHeight = window.innerHeight; // winHeight에 윈도우 콘텐츠의 높이값을 넣어줍니다.
    var result2 = "윈도우 콘텐츠의 영역 width : " + winWidth + "px, height : " + winHeight + "px 입니다." // 문자열 결과값을 만들어줍니다.
    return result2; // result2를 반환합니다.
}

var winSize = getSize() // winSize 변수를 선언하고 getSize() 의 반환값을 할당받습니다.
console.log(winSize); // 윈도우 콘텐츠의 영역 width : 1261px, height : 937px 입니다.
// 실수로 함수 바깥 console에서 문자열 결과값을 내려다 골머리를 앓았습니다. 함수 내에서 하면 되는건데... 아직 많이 부족하다는 것을 깨달았습니다 ㅜ